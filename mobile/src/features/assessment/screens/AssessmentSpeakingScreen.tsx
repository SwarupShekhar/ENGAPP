...
export default function AssessmentSpeakingScreen({ navigation, route }: any) {
  ...
  const startAssessment = async () => {
    try {
      ...
      if (perm.status !== "granted") {
        Alert.alert(
          "Permission Required",
          "Microphone access is needed for the assessment.",
        );
        return;
      }
  ...
  const handleSubmit = async (uri: string) => {
    if (!assessmentId) {
      Alert.alert(
        "Error",
        "Assessment session not initialized. Please try restarting.",
      );
      return;
    }
    ...
    try {
      ...
      const res = await assessmentApi.submitPhase(
        assessmentId,
        phase,
        base64,
        attempt,
      ...
      if (res.nextPhase) {
        ...
        if (res.imageUrl)
          setContent({ imageUrl: res.imageUrl, level: res.imageLevel });
