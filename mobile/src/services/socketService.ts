...
class SocketService {
   ...
   * Accepts a getToken function so that a fresh token is fetched on every
  ...
  connect(getToken: (() => Promise<string | null>) | string, userId?: string) {
    ...
    // Support both old static token and new function signature
    if (typeof getToken === "function") {
      this.tokenFetcher = getToken;
    } else {
      // Legacy: wrap static token in a function
      const staticToken = getToken;
      this.tokenFetcher = async () => staticToken;
    }

    const tokenFetcher = this.tokenFetcher;

    this.socket = io(`${API_URL}/chat`, {
      // auth as a function: socket.io calls this on EVERY connection attempt
      auth: async (cb) => {
        try {
    ...
    this.socket.on("new_message", (data) => {
      ...
      if (
        ...
        data.message.senderId !== this.localUserId
      ) {
  ...
  onNewMessage(callback: SocketCallback) {
    this.on("new_message", callback);
  }
  offNewMessage(callback: SocketCallback) {
    this.off("new_message", callback);
  }

  onUserTyping(callback: SocketCallback) {
    this.on("user_typing", callback);
  }
  offUserTyping(callback: SocketCallback) {
    this.off("user_typing", callback);
  }

  onMessagesRead(callback: SocketCallback) {
    this.on("messages_read", callback);
  }
  offMessagesRead(callback: SocketCallback) {
    this.off("messages_read", callback);
  }

  onPresenceUpdate(callback: SocketCallback) {
    this.on("presence_update", callback);
  }
  offPresenceUpdate(callback: SocketCallback) {
    this.off("presence_update", callback);
  }

  onIncomingCall(callback: SocketCallback) {
    this.on("incoming_call", callback);
  }

  offIncomingCall(callback: SocketCallback) {
    this.off("incoming_call", callback);
  }

  onFriendRequest(callback: SocketCallback) {
    this.on("friend_request", callback);
  }

  offFriendRequest(callback: SocketCallback) {
    this.off("friend_request", callback);
  }

  onCallStatusUpdate(callback: SocketCallback) {
