            ...
            >
                ...
                <Animated.View entering={FadeInDown.delay(100).springify()} style={styles.scoreCard}>
                    ...
                    >
                        ...
                        <View style={styles.scoreCircle}>
                            <Text style={styles.scoreCircleText}>{Math.round(overallScore)}</Text>
                        </View>
                        {result?.confidence_metrics && (
                            <View style={styles.confidenceContainer}>
                                <MaterialCommunityIcons name="shield-check" size={16} color="white" />
                                <Text style={styles.confidenceText}>
                                    Confidence: {Math.round(result.confidence_metrics.overall_confidence.score)}%
                                </Text>
                            </View>
                        )}
                    </LinearGradient>
                ...
                <View style={styles.section}>
                    ...
                    <View style={styles.planCard}>
                        {Object.entries(result?.skillBreakdown || {}).map(([skill, metrics]: [string, any], index) => (
                            <View key={skill} style={{ marginBottom: index === 3 ? 0 : 20 }}>
                                <View style={styles.skillHeader}>
                                    <View style={styles.skillIconName}>
                                        <Ionicons 
                                            name={
                                                skill === 'pronunciation' ? 'mic' :
                                                skill === 'fluency' ? 'speedometer' :
                                                skill === 'grammar' ? 'library' : 'bookmarks'
                                            } 
                                            size={18} 
                                    ...
                                    <Text style={styles.skillScore}>
                                        {Math.round(Object.values(metrics).reduce((a: any, b: any) => a + b, 0) as number / Object.values(metrics).length)}%
                                    </Text>
                                </View>
                                
                                {Object.entries(metrics).map(([subSkill, score]: [string, any]) => (
                                    <View key={subSkill} style={styles.subMetricRow}>
                                        <Text style={styles.subMetricLabel}>
                                            {subSkill.replace(/([A-Z])/g, ' $1').trim().replace(/^./, str => str.toUpperCase())}
                                        </Text>
                                            ...
                                            <View 
                                                style={[
                                                    ...
                                                    { 
                                                        ...
                                                        backgroundColor: score > 70 ? theme.colors.primary : score > 40 ? theme.colors.secondary : theme.colors.error 
                                                    }
                                                ]} 
                                            />
                                        </View>
                                        <Text style={styles.subMetricValue}>{Math.round(score)}%</Text>
                                    </View>
                                ))}
                            </View>
                        ))}
                        ...
                        <View style={styles.planCard}>
                            <View style={styles.breakdownGrid}>
                                <View style={styles.breakdownItem}>
                                    ...
                                    <Text style={styles.breakdownValue}>{Math.round(result.fluencyBreakdown.speech_flow)}%</Text>
                                </View>
                                <View style={styles.breakdownItem}>
                                    <Text style={styles.breakdownLabel}>Connected Speech</Text>
                                    <Text style={styles.breakdownValue}>
                                        {Math.round(result.fluencyBreakdown.connected_speech || result.fluencyBreakdown.naturalness)}%
                                    </Text>
                                </View>
                                <View style={styles.breakdownItem}>
                                    <Text style={styles.breakdownLabel}>Prosody</Text>
                                    <Text style={styles.breakdownValue}>{Math.round(result.fluencyBreakdown.prosody)}%</Text>
                                </View>
                                <View style={styles.breakdownItem}>
                                    <Text style={styles.breakdownLabel}>Pace Control</Text>
                                    <Text style={styles.breakdownValue}>{Math.round(result.fluencyBreakdown.pace_control)}%</Text>
                                </View>
                            </View>
                            ...
                            <View style={styles.azureComparison}>
                                ...
                                <Text style={styles.azureComparisonText}>
                                    Adjusted from Azure's raw fluency of {Math.round(result.rawAzureMetrics?.fluency || 0)}% for realism.
                                </Text>
                            </View>

                            {/* Connected Speech Examples */}
                            {(result.fluencyBreakdown.examples?.linking_detected?.length > 0 || 
                              result.fluencyBreakdown.examples?.reductions_detected?.length > 0) && (
                                <View style={styles.examplesContainer}>
                                    <Text style={styles.examplesTitle}>Connected Speech Patterns</Text>
                                    
                                    {result.fluencyBreakdown.examples?.linking_detected?.map((ex: string, i: number) => (
                                        <View key={`link-${i}`} style={styles.exampleRow}>
                                            <Ionicons name="link-outline" size={14} color={theme.colors.success} />
                                            <Text style={styles.exampleText}>Linking: {ex}</Text>
                                        </View>
                                    ))}
                                    
                                    {result.fluencyBreakdown.examples?.reductions_detected?.map((ex: string, i: number) => (
                                        <View key={`red-${i}`} style={styles.exampleRow}>
                                            <Ionicons name="trending-down-outline" size={14} color={theme.colors.primary} />
                ...
                <Animated.View entering={FadeInDown.delay(350).springify()} style={styles.section}>
                    ...
                    {result?.benchmarking && (
                        <BenchmarkCard data={result.benchmarking} />
                    )}
                    
                    {result?.readiness && (
                        <ReadinessCard data={result.readiness} />
                    )}
