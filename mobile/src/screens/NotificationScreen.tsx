...
export default function NotificationScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      fetchRequests();
      // Point 5: Mark requests as seen (clear badge)
      AsyncStorage.setItem("@home_notif_cache", JSON.stringify(0));
    }, []),
  );
  ...
  const handleAccept = async (id: string, name: string) => {
    try {
      ...
      const msg =
        error?.response?.data?.message || error.message || "Failed to request";
