...
export default function HomeScreen() {
    ...
    useCallback(() => {
      ...
      const fetchData = async () => {
        try {
          ...
          const cachedNotif = await AsyncStorage.getItem("@home_notif_cache");
          if (cachedStats) {
          ...
          if (cachedNotif) {
            setNotifCount(JSON.parse(cachedNotif));
          }

          // 2. Fresh data
          const [statsData, unreadData, pendingReqs] = await Promise.all([
            userApi.getStats(),
  ...
  const displayData = {
    ...
    overallScore: stats?.feedbackScore
      ? Math.min(100, Math.round(stats.feedbackScore))
      : 0,
  };
