...
export class UsersService {
  ...
  async getUserStats(userId: string) {
    ...
    const latestConversationSession =
      await this.prisma.conversationSession.findFirst({
        ...
        include: {
          analyses: {
            where: {
              participantId: { in: await this.getParticipantIds(userId) },
            }, // Helper needed or simplified
            take: 1,
          },
        },
      });
    ...
    const conversationDate =
      latestConversationSession?.createdAt || new Date(0);
    ...
    if (
      isconversationNewer &&
      latestConversationSession?.analyses?.[0]?.scores
    ) {
      ...
      feedbackScore = Math.round(
        (fluencyScore + pronScore + vocabScore + grammarScore) / 4,
      );
    }
    ...
    return history.map((session) => {
      ...
      const score = Math.round(
        ((p2.finalFluencyScore || 0) +
          ...
          (p3.grammarScore || 0) +
          this.mapCEFRToScore(p3.vocabularyCEFR || 'A1')) /
          4,
      );
