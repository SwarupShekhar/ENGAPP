...
export class AzureService {
    ...
    async analyzeSpeech(audioUrl: string, referenceText: string = '', audioBase64?: string): Promise<{
        transcript: string;
        ...
        try {
            ...
            if (!transcript) {
                ...
                const transResponse = await lastValueFrom(
                    this.httpService.post(`${this.aiEngineUrl}/api/transcribe`, transcribePayload)
                );
