...
export class TieredMatchmakingService {
  ...
  async findMatch(
    userId: string,
    structure: string,
  ): Promise<{ partnerId: string; sessionId: string } | null> {
    ...
    const criteria: MatchCriteria = {
      ...
      reliabilityMin: this.getMinReliabilityForUser(
        user.reliability?.reliabilityScore || 100,
      ),
    ...
    while (Date.now() - startTime < TIMEOUT_MS) {
      ...
      const partnerId = await this.searchForMatch(
        dbUserId,
        structure,
        criteria,
        mode,
      ...
      if (partnerId) {
        ...
        if (dbUserId && dbPartnerId) {
          ...
          // Cleanup queues
          await this.removeFromQueue(dbUserId);
          await this.removeFromQueue(dbPartnerId);
  ...
  private async searchForMatch(
    ...
    if (mode === 'strict') {
      filteredCandidates = candidates.filter(
        (c) => c.reliability?.tier === criteria.tierMin,
      );
    } else if (mode === 'medium') {
  ...
  private async removeFromQueue(userId: string) {
    ...
    // Or check user meta.
    const meta = await this.redisService
      .getClient()
      .hgetall(`user:${userId}:meta`);
    if (meta && meta.level) {
      await this.redisService
        .getClient()
        .lrem(`queue:${meta.level}`, 0, userId);
  ...
  private calculateMatchScore(candidate: any, criteria: MatchCriteria): number {
    ...
    const reliabilityDiff = Math.abs(
      (candidate.reliability?.reliabilityScore || 100) -
        (criteria.reliabilityMin || 100),
    );
